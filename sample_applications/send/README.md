...
|---------|-----|----------|-------|
|-s|Sender number|Your Short Number or other phone number|*NOT IMPLEMENTED*|
|-r|Recipient number|The recipient’s mobile terminal number|*NOT IMPLEMENTED*|
...
|[-m]|Message|The message<p>Use -m as the last parameter in the command. Send includes everything after the –m in the message.|*NOT IMPLEMENTED*|
...
|[-f]|Filename|The name of the file that contains the message<p>The message is sent from the file specified in the parameter. If you omit the –m or –f parameters from the command, Send sends the text from the console as the message.|*NOT IMPLEMENTED*|
